**Detached HEAD 的定義**：HEAD 直接指向某個 commit，而不是透過分支名稱指向。當直接 checkout 到 commit、tag，或是遠端分支時就會發生。這裡要特別留意的是，為什麼說**遠端分支**會是 detached HEAD？因為遠端分支 (如 origin/main) 是唯讀的本地鏡像，代表遠端狀態的快照，不能直接在上面進行開發工作
